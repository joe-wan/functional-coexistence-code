...
fct.plot.fade <- fct.plot + 
  fade.box(xlim=trad.to.log(fct.x), ylim=fct.y)
if (show.plots) print(fct.plot.fade)
...
fct.plot.wide <- plot.fct.df(fct.data.wide, ylim=fct.y, xlim=trad.to.log(fct.x.wide), 
      log.version=T,
      zero.linetype=axes.lt, optimal.linetype=highlight.lt,
      xlab=nd.label, ylab=fr.label) +
